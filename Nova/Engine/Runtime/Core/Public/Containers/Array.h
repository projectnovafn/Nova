...
public:
    ...
    TArray<ElementType> FilterByPredicate(Predicate Pred) const
    {
        ...
        {
            if (Pred(*reinterpret_cast<const ElementType*>(Data)))
            {
                FilterResults.Add(*reinterpret_cast<const ElementType*>(Data));
            }
        }
...
public:
     ...
     */
    FORCEINLINE int32 Emplace(const ElementType& Element)
    {
        const int32 Index = AddUninitialized(1);
        FMemory::Memcpy((uint8*)AllocatorInstance.GetAllocation() + Index * GetElementSize<ElementType>(), &Element, GetElementSize<ElementType>());

        return Index;
    }

    FORCEINLINE int32 EmplaceRef(ElementType* Element)
    {
        const int32 Index = AddUninitialized(1);
        FMemory::Memcpy((uint8*)AllocatorInstance.GetAllocation() + (Index * GetElementSize<ElementType>()), Element, GetElementSize<ElementType>());
    ...
    FORCEINLINE int32 AddRef(ElementType* ItemRef)
    {
        CheckAddress(ItemRef);
        return EmplaceRef(ItemRef);
    }
