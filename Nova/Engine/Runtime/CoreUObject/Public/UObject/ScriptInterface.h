...
public:
	...
	template <
		typename U,
		typename = std::enable_if_t<std::is_convertible_v<U, UObjectType*>>
	>
	FORCEINLINE TScriptInterface(U&& Source)
	{
		...
		if constexpr (std::is_base_of_v<InInterfaceType, std::remove_pointer_t<std::remove_reference_t<U>>>)
		{
	...
	template <
		...
		typename = std::enable_if_t<std::is_convertible_v<OtherInterfaceType*, InInterfaceType*>>
	>
	...
	{
		*this = TScriptInterface(std::forward<U>(Source));
		return *this;
	}
	...
	template <
		...
		typename = std::enable_if_t<std::is_convertible_v<OtherInterface*, InInterfaceType*>>
	>
	 ...
	 * Returns the ObjectPointer contained by this TScriptInterface
	 */
	FORCEINLINE UObjectType*& GetObjectRef()
	{
		return (UObjectType*)FScriptInterface::GetObjectRef();
	}
